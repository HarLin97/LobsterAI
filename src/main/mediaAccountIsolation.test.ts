...
describe('canCommitAuthRefresh', () => {
  test('commits only to the account generation and refresh token that started the request', () => {
    const refresh = {
      ...enterpriseScope,
      refreshToken: 'refresh-a',
    };

    expect(canCommitAuthRefresh(refresh, { ...enterpriseScope }, 'refresh-a')).toBe(true);
    expect(canCommitAuthRefresh(refresh, {
      ownerAccountKey: 'enterprise:6:1002',
      accountGeneration: enterpriseScope.accountGeneration,
    }, 'refresh-a')).toBe(false);
    expect(canCommitAuthRefresh(refresh, {
      ownerAccountKey: enterpriseScope.ownerAccountKey,
      accountGeneration: enterpriseScope.accountGeneration + 1,
    }, 'refresh-a')).toBe(false);
    expect(canCommitAuthRefresh(refresh, { ...enterpriseScope }, 'refresh-b')).toBe(false);
  });
});
